...
import numpy as np
from pso.tools import func_transformer
...
class PSO():
    ...
    max_iter : int 最大的迭代次數
        Max of iter iterations
    lb : array_like  每個變量的下界
        The lower bound of every variables of func
    ub : array_like 每個量的上界
        The upper bound of every variables of func
    constraint_eq : tuple 等式約束
        equal constraint. Note: not available yet.
    constraint_ueq : tuple 不等式約束
        unequal constraint
    Attributes
    ----------------------
    ...
    gbest_x : array_like, shape is (1,dim) 历史上所有粒子的最佳位置
        general best location for all particles in history
    gbest_y : float 历史上所有粒子的最佳图像
        general best image  for all particles in history
    gbest_y_hist : list 每个迭代的gbest_y
    ...
    def update_gbest(self):
        ...
        print("===============当前最优==================")
        print('best_x is ', self.gbest_x, 'best_y is', self.gbest_y)
        print("========================================")
    ...
    def run(self, max_iter=None, precision=1e-7, N=20):
        ...
        c = 0
        for iter_num in range(self.max_iter - 1):
            self.update_V()
            self.recorder()
            self.update_X()
            self.cal_y()
