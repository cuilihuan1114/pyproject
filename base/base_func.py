...
class Batch(object):
    ...
    def next_batch(self):
        """Return the next `batch_size` examples from this dataset set."""
        ...
        if start + self.batch_size > self._num_examples:
            ...
            # Shuffle the dataset 对数据进行随机化处理
            if self.shuffle:
...
class Batch1(object):
    ...
    def next_batch(self):
        ...
        if self._epochs_completed == 0 and start == 0 and self.shuffle:
            ...
            np.random.shuffle(perm0)
            self._images = self.images[perm0]
            self._images1 = self.images1[perm0]
            if self.exit_y: self._labels = self.labels[perm0]
        # Go to the next epoch
        if start + self.batch_size > self._num_examples:
            # Finished epoch
            ...
            if self.shuffle:
                ...
                np.random.shuffle(perm)
                self._images = self.images[perm]
                self._images1 = self.images1[perm]
                if self.exit_y: self._labels = self.labels[perm]
            # Start next epoch
            start = 0
            self._index_in_epoch = self.batch_size - rest_num_examples
            ...
            if self.exit_y:
                labels_new_part = self._labels[start:end]
                return np.concatenate((images_rest_part, images_new_part), axis=0), np.concatenate(
                    (images_rest_part1, images_new_part1), axis=0), \
                       np.concatenate(
                           (labels_rest_part, labels_new_part), axis=0)
            else:
                return np.concatenate((images_rest_part, images_new_part), axis=0), np.concatenate(
            ...
            if self.exit_y:
                return self._images[start:end], self._images1[start:end], self._labels[start:end]
            else:
                return self._images[start:end], self._images1[start:end]
...
class Summaries(object):
    def __init__(self,
                 ...
                 sess):
        print()  # 打印模型
        # write_path = '../tensorboard/' + file_name 定义 FileWriter 用于记录 merge
...
class Accuracy(object):
    ...
    def accuracy(self):
        ...
        return tf.reduce_mean(tf.cast(tf.equal(pre_lables, data_lables), tf.float64))  # 改
...
def run_sess1(classifier, datasets, datasets1, filename, load_saver=''):
    X_train, Y_train, X_test, Y_test = datasets
    X_train1, Y_train1, X_test1, Y_test1 = datasets1

    sess = tf.Session()
    sess.run(tf.global_variables_initializer())  # 初始化变量

    classifier.tbd = True  # 自己加的
    if classifier.tbd:
        summ = Summaries(filename, sess=sess)
    else:
        summ = None
    ...
    return classifier.show_result(filename)
    #
    # if classifier.tbd:
