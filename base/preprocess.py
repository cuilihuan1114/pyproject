...
import numpy as np
import os
from sklearn import preprocessing  # 0-1 编码
from sklearn.model_selection import StratifiedShuffleSplit  # 随机划分，保证每一类比例相同

def prepro(d_path, length=864, number=200, normal=True, rate=[0.7, 0.2, 0.1], enc=True, enc_step=28):
    """
    对数据进行预处理,返回 train_X, train_Y, valid_X, valid_Y, test_X, test_Y 样本.
    ...
    def capture(original_path):
        ...
        for i in filenames:
            ...
            file = loadmat(file_path)  # 导入源数据
            file_keys = file.keys()
            for key in file_keys:
                if 'DE' in key:      # 选择驱动端加速度计数据
                    files[i] = file[key].ravel()  # 将多维数组拉平为一维
                if 'temp' in key:      # 选择驱动端加速度计数据
                    files[i] = file[key].ravel()  # 将多维数组拉平为一维
        return files
    ...
    def scalar_stand(Train_X, Test_X):       # 用训练集标准差标准化训练集以及测试集
        scalar = preprocessing.StandardScaler().fit(Train_X)
        Train_X = scalar.transform(Train_X)
        Test_X = scalar.transform(Test_X)
        return Train_X, Test_X

    def valid_test_slice(Test_X, Test_Y):    # 将测试集切分为验证集合和测试集
...
if __name__ == "__main__":
    # path = r'C:\Users\MSI\PycharmProjects\pythonProject\CWRU\data\0HP'
    path = r'F:\a_数据备用\AE_workspace\AE_data\ae_test_data\test_2'
    ...
    savemat("1data_process.mat", {'train_X': train_X,'train_Y': train_Y,
